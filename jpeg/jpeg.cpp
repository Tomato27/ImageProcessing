...
#define raise_q 4 //q_tableに乗ずる値、大きいほど圧縮率も大きく画質は荒くなる

const char readFileName[]  = "lenna_uchar_256-256.raw";
const char writeFileName[] = "jpeg.raw";
...
float sixty_eight(float* in){/////deEncopy_decoderの書き込みで使うlo
    REP(j1,0,8){
        cout<<i1;
    }
  }
      ...
      REP(x,0,8){//それぞれの8*8走査
        REP(y,0,8){
          if(g[(x+a*8)*H+y+8*b]>0){
            tmp[x*8+y]=(int)(0.5+(g[(x+a*8)*H+y+8*b]/raise_q)/q_table[x][y]);
          }
          else{
            tmp[x*8+y]=(int)((g[(x+a*8)*H+y+8*b]/raise_q)/q_table[x][y]-0.5);
          }
        }
      }
      //Zigzag scan
    ...
    { //DC成分の処理
        ...
        REP(i,0,12){/////SSSSの確定
          ...
          REP(j,0,dc_length_table[i]){
            check_bits=true;
            if(code[a+j]==dc_code_table[i][j]){//16桁の場合も考慮せねば
              b++;
          ...
          if(check_bits==true){
            //追加コードのcheck
            if(code[a+1]==1){//正の値 j,i?
              REP(k,1,i+1){
                in[0]+=pow(2,i-k)*code[a+k];
              }
            }
            else{//負の値
              REP(k,1,i+1){
                in[0]+=-1*pow(2,i-k)*(code[a+k]^1);
              }
            }
            a+=i+1;//+1必要かも
    ...
    {   //AC成分
        ...
        while(flag_ac){
          REP(i,0,176){
            ...
            REP(j,0,ac_length_table[i]){
              if(code[a+j]!=ac_code_table[i][j]){
                check_bits=false;
                break;
              }
              else{
              ...
              if(i==0){/////////↓1010の処理
                ...
                if(code[a]==1){
                  REP(k,1,i%11+1){///size(SSSS) a
                    in[out_index]+=pow(2,i%11-k)*code[a];
                    a++;
                  }
                  out_index++;
                  break;
                  ...
                  REP(k,1,i%11+1){///run、sizeの後の数字を代入
                    in[out_index]+=-1*pow(2,i%11-k)*(code[a]^1);
                    a++;
                  }
                  out_index++;
                  break;
  ...
  REP(a,0,32){
    REP(b,0,32){
      ...
      REP(x,0,8){//それぞれの8*8走査
        REP(y,0,8){
          if(tmp[x*8+y]>0){
            h[(a*8+x)*H+b*8+y]=(int)(tmp[x*8+y]*raise_q*q_table[x][y]-0.5);
          }
          else{
            h[(a*8+x)*H+b*8+y]=(int)(tmp[x*8+y]*raise_q*q_table[x][y]+0.5);
          }
